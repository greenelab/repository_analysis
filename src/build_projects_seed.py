    ...
    for p in sorted(dir_path.glob("*.json")):
        ...
        try:
            ...
            if isinstance(data, list) and data:
                first = data[0]
                if isinstance(first, dict) and {"coreProject", "owner", "name"}.issubset(first.keys()):
                    return p
        except Exception:
            continue
    sys.exit(f"ERROR: Could not find repos JSON in {dir_path} with keys coreProject/owner/name.")
...
def build_rows(active_map: Dict[str, str], repos_list: List[dict]) -> List[Tuple[str, str, str, str, str]]:
    """
...
def main():
    ap = argparse.ArgumentParser(description="Generate projects_seed.csv from a private GitHub repo (SSH auth).")
    ap.add_argument("--repo", default=OWNER_REPO, help="owner/repo of the private repository")
    ap.add_argument("--branch", default=BRANCH, help="branch to checkout (default: main)")
    ap.add_argument("--subdir", default=SUBDIR, help="subdirectory to sparse-checkout (contains the JSON files)")
    ap.add_argument("--output", default="data/projects_seed.csv", help="output CSV path")
    args = ap.parse_args()
    ...
    with tempfile.TemporaryDirectory(prefix="cfde_") as tmpd:
        ...
        run_git([
            "clone", "-n", "--depth=1", "--filter=tree:0",
            f"git@github.com:{args.repo}.git", str(repo_dir)
        ])

        print("Setting sparse-checkout and checking out...")
        run_git(["-C", str(repo_dir), "sparse-checkout", "set", "--no-cone", f"/{args.subdir}"])
        ...
        if skipped:
            print(f"Note: {len(skipped)} active project(s) had no repo entries and were skipped.")
